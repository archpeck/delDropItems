...
1. Скачайте файл .jar плагина.
2. Поместите файл .jar в директорию `plugins` вашего сервера Minecraft.
3. Запустите или перезапустите сервер, чтобы сгенерировать файл конфигурации по умолчанию.
4. Отредактируйте файл `config.yml`, чтобы настроить параметры по своему усмотрению.
5. Используйте команду `/ddi reload`, чтобы применить изменения без перезапуска сервера.
